...
3. Spawn the **interviewer** from `tests/prompts/interviewer.md`: fill `<kickoff>`. It loads `SKILL.md` itself — that is its whole setup; add nothing else.
4. Relay **verbatim**: pass only the other party's exact words — no labels ("The person said:"), no framing, no instructions ("reply now", "return only your reply"). Each side sees only the other's words. Log the conversation to <run_dir>/transcript.md as `interviewer:` / `employee:`. One **turn** is one interviewer question and the answer to it (the kickoff and a closing summary aren't turns); stop when the interviewer closes, or at <turns> turns (hard cap).
