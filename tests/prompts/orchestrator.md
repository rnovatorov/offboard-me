...
4. Relay each turn **verbatim**: pass only the other party's exact words — no labels ("The person said:"), no framing, no instructions ("reply now", "return only your reply"). Each side sees only the other's words. Log each turn to <run_dir>/transcript.md as `interviewer:` / `employee:`. Stop when the interviewer closes, or at <turns> turns (hard cap).
